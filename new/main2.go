...
func imagesHandler(w http.ResponseWriter, r *http.Request) {
	...
	if urlOrigem == "" {
		http.Error(w, "O parâmetro 'url_origem' é obrigatório", http.StatusBadRequest)
		return
	}

	// Faz o scraping e gera a lista de URLs
	urls, err := scrapeImages(urlOrigem)
	if err != nil {
...
// Função para pegar o Filename e retornar o nome do video
func getVideoName(fileName string) string {
	// Extrai a última parte após a última barra
	baseName := path.Base(fileName)

	// Substitui underscores por espaços
	videoName := strings.ReplaceAll(baseName, "_", " ")
